...
(function () {
  ...
  // Inline SVG avoids an extra HTTP request for an icon font.
...
(function () {
  ...
  var pagesData = null; // loaded once on first open, then cached across searches
  var activeIndex = -1;
  ...
  function search(query) {
    ...
    // Score and filter: title match outweighs body, body outweighs section.
    var scored = [];
  ...
  // Use capture phase so Escape/arrow keys are intercepted before browser defaults
  // (e.g. some browsers clear the input on Escape before JS can handle it).
  ...
  // Event: click result item to navigate.
  ...
  // Event: debounced live search on input.
